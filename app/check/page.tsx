...
export default function CheckPage() {
  ...
  // OCR review dialog — pops up the moment a scan finishes so the user can
  // verify / edit / retry the extracted text before it enters the intake box.
  ...
  // Run OCR on a file and surface the result in the review dialog (which is
  // already open during a retry). Shared by the initial upload and "Retry scan".
  ...
  // Upload → OCR → show the review dialog for confirmation.
  function handleImageUpload(file: File) {
    setOcrFile(file);
    runOcr(file);
  }

  // User confirms the scanned text → flow it into the intake box and close.
  function confirmOcr() {
  ...
  // User says the scan is wrong → re-run OCR on the same image.
  function retryOcr() {
    ...
    <header className="flex items-center justify-between gap-3">
      <div>
        <Link href="/dashboard" className="inline-block">
          <h1 className="font-display text-3xl font-bold tracking-tight text-ink-900 transition-colors hover:text-brand-600">
